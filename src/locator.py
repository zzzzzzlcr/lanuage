...
class FieldLocator:
    ...
    def _candidates_custom_select(self, field, frame_id, container) -> List[dict]:
        ...
        js = (
            ...
            # Walk up from label to find custom dropdown trigger (NOT native <select>)
            f"var p=labels[i];var trigger=null;"
            f"for(var lv=0;lv<4&&!trigger;lv++){{"
            f"p=p.parentElement;if(!p)break;"
            f"var t=p.querySelector('[role=combobox],.ant-select-selector,[class*=select__control]');"
